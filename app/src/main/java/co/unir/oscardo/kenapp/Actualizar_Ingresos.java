...
public class Actualizar_Ingresos extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_actualizar__ingresos);
    }

    public void Actualizar_Ingresos(View view) {
        ...
        for (Clase_Ingreso CI:
            ...
            int val = Nro + 1;
            rb[Nro].setText("Nro: " + val + " ID:" + CI.getId() +  " Concepto: " + CI.getConcepto() + " Valor: " + CI.getValor() + "\n");
            rb[Nro].setId(Nro);
            rb_datos_lista.addView(rb[Nro]);
            Nro++;
        }
        Button btn_Actualiza = (Button)findViewById(R.id.btn_Actualizar_Ingresos);
        btn_Actualiza.setVisibility(View.VISIBLE);
        DB.insertar_log("Actualizar_Ingresos");
    }
    ...
    public void Actualiza_Ingresos(View view) {
        ...
        if(idResp != -1) {
            ...
            concepto.setText((Ingresos.get(valorIngreso).getConcepto()));
            concepto.setVisibility(View.VISIBLE);
            tv_concepto.setVisibility(View.VISIBLE);

            valor.setText((String.valueOf(Ingresos.get(valorIngreso).getValor())));
            ...
            idResp = Integer.valueOf(Ingresos.get(valorIngreso).getId());
            DB.insertar_log("Actualiza_Ingresos");
        }else{
            Toast.makeText(this, R.string.Actualiza_ingreso_faltaRadio, Toast.LENGTH_LONG).show();
        }
    }

    public void Actualizar_ing(View view) {
        ...
        CI.setId(idResp);
        CI.setConcepto(et_actual_concepto.getText().toString());
        CI.setValor(Float.valueOf(et_actual_valor.getText().toString()));
        Boolean rpt = DB.actualizar_ingresos(CI);
        DB.insertar_log("Actualizar");
        if(rpt){
            Intent IrConfiguracion = new Intent(this, Configuracion.class);
            startActivity(IrConfiguracion);
        }else{
            Toast.makeText(this, R.string.Actualiza_error_Ingreso, Toast.LENGTH_LONG).show();
        }
    }
}
